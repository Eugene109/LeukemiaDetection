...
BOOL CellListView::InitInstance(int x, int y, int w, int h, HINSTANCE hInstance, int nCmdShow, HWND parent) {
    ...
    if (!hWnd)
    {
        OutputDebugStringW(L"CellListView FAILED");
    }
    else {
        OutputDebugStringW(L"CellListView INIT");
    }

    return TRUE;
}

ATOM CellListView::RegisterClasses(HINSTANCE hInstance)
{
    WNDCLASSEXW wcex;
    ...
    wcex.lpszMenuName = 0;
    wcex.lpszClassName = L"Cell List";
...
LRESULT CALLBACK CellListView::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_COMMAND:
...
BOOL CellListView::Paint(HWND hWnd) {
    PAINTSTRUCT ps;
    ...
    LPCSTR error_msg = "Cell List View";
